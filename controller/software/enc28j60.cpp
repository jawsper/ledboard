...
uint16_t ENC28J60::packetReceive() {
    ...
    if (readRegByte(EPKTCNT) > 0) {
        /// set read pointer to start of next packet
        ...
        /// read the ENC header of the packet
        readBuf(sizeof header, (byte*) &header);

        /// set next packet pointer to next packet
        gNextPacketPtr  = header.nextPacket;
        len = header.byteCount - 4; //remove the CRC count
        /// limit read to buffer size
        if (len>bufferSize-1)
            len=bufferSize-1;
        /// status & 0x80 set means received OK
        if ((header.status & 0x80)==0)
            len = 0;
        else
            /// READ ALL THE PACKET (not the CRC)
            readBuf(len, buffer);
        /// zero terminate the buffer
        buffer[len] = 0;
        /// Page 49, chapter 7.2.4:
...
uint16_t ENC28J60::specialPacketReceive(bool (*isMySpecialPacket)(byte*, uint16_t), void (*specialPacketReceive)(byte))
{
    uint16_t len = 0;
    if (readRegByte(EPKTCNT) > 0) {
        /// set read pointer to start of next packet
        ...
        {
            byte header2[ETH_HEADER_LEN + IP_HEADER_LEN + UDP_HEADER_LEN];
            int header2_len = sizeof(header2);

            readBuf(header2_len, (byte*)&header2); // usually enough for udp header

            // bool is_ip = header2[ETH_TYPE_H_P] == ETHTYPE_IP_H_V && header2[ETH_TYPE_L_P] == ETHTYPE_IP_L_V;
            // bool is_udp = header2[IP_PROTO_P] == IP_PROTO_UDP_V;
            // uint16_t port = (header2[UDP_DST_PORT_L_P] << 8) | (header2[UDP_DST_PORT_H_P] & 0xFF);

            if((*isMySpecialPacket)(header2, header2_len))
            {
                uint16_t udp_length = *((uint16_t*)(header2 + UDP_LEN_H_P));
                enableChip();
                xferSPI(ENC28J60_READ_BUF_MEM);
                for(int i = 0; i < udp_length; i++)
                {
                    xferSPI(0x00);
                    (*specialPacketReceive)(SPDR);
                }
                disableChip();
            }
            else
            {
                ...
                if (len>bufferSize-1)
                    len=bufferSize-1;
                memcpy(buffer, (byte*)&header2, header2_len);
                /// READ ALL THE PACKET (not the CRC)
                readBuf(len - header2_len, buffer + header2_len);
            }
        }
        /// zero terminate the buffer (WAAROM DAN?)
        buffer[len] = 0;
        /// Page 49, chapter 7.2.4:
        /// "After the host controller has processed a packet (or part
        /// of the packet) and wishes to free the buffer space used
